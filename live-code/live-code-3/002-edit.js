...
function arrayMerge(firstData, secondData) {
  for (var i = 0; i < secondData.length; i++) { // lopping sepanjang array secondData
    var cek = true // apabila benar true
    for (var j = 0; j < firstData.length; j++) { // looping sepanjang array firstData
      if (secondData[i] === firstData[j]) { // mengecek kondisi apakah secondData sama dengan firstData
        cek = false // apabila sama maka hasilnya false
      }
    }
    if (cek === true) { // jika benar maka tampung data ke dalam secondData
...
// Test cases
  ...
  console.log(arrayMerge(['king', 'devil jin', 'akuma', 'eddie'], ['eddie', 'steve', 'geese']));
